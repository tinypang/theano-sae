...
def evaluate_lenet5(path,input_type,nfolds=1,learning_rate=0.1, n_epochs=200,dimx=28,dimy=28,pcancomps=0,nceps=33,nkerns=[20, 50], batch_size=500, results_log='CNNresults.txt',reinput=None):
    ...
    log.write('number of folds is {0}\n'.format(nfolds))
    ...
    for fold in range(0,n):
        ...
        log.write('fold: {0}\n'.format(fold))
        datasets = split_dataset(dataset,n_classes) 
        ...
        print >> sys.stderr, ('The code for file ' +
                              ...
                              ' ran for %.2fm' % ((end_time - start_time) / 60.)+'for fold'+fold)
        log.write('The code for file ' +
                              os.path.split(__file__)[1] +
                              ' ran for %.2fm' % ((end_time - start_time) / 60.)+'for fold'+fold+'\n')
        fold_scores.append([1- (best_validation_loss * 100.),1-(test_score * 100.)])
